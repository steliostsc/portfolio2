...
export const getAllVideoProjects = (): VideoProject[] => {
  return allVideoProjects; // keep the array as-is
};
...
export async function getTikTokThumbnail(videoLink: string): Promise<string | null> {
  try {
    const api = `https://www.tiktok.com/oembed?url=${encodeURIComponent(videoLink)}`;
...
export function getTikTokEmbedUrl(videoLink: string) {
  // Extract the TikTok video ID from the link
  // Example video link: https://www.tiktok.com/@username/video/7568113754597084438
